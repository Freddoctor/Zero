...
<section class="list-dealer">
    <ul>
        <li v-for="(item,index) in brokerLists" :key="index">
            <div class="login">
                <img :src="item.logo">
                <ins class="icon-sumer"><img src="" alt=""> </ins>
            </div>
            <article class="article">
                <h3 :class="{'hot':item.isVip == 1}">{{item.agentName}} <img v-if="item.isVip == 1" src="@/assets/images/guanfang2x.png"></h3>
                <aside class="power-em">
                    <em v-for="(part,idx) in item.keyWords">
                      {{part}}
                    </em>
                </aside>
                <p>
                ...
                <a data-admin="tgg" :href="item.evaluation.url" v-if="item.evaluation">
                    <footer>
            ...
            handleFun() {
                ...
                if (scrollTop + clientHeight >= htmlHeight) {
                    if (this.loadOption.isLoad && this.loadOption.result) {
                        this.getBrokerList("/api/comment/getAgentList.json", this.loadOption);
                    }
                }
            }
    },
